...
from getdataset_RML201801a import ModulationRecognitionDataset_RML201801A
from modulation_dict import get_modulation_array_list, filter_classes_str_dic
...
dataset_type = 'RML2018.01a'


select_every = 1  # 建议为1、128、256
train_ratio = 0.9
print('Reading RML2018.01a')
train_filename = './GOLD_XYZ_OSC.0001_1024.hdf5'
snr_list = list(range(-20, 30 + 1, 2))
RML201801a_modulation_Easy_list = [0,1,2,3,4,7,8,9,18,19,20,21,22,23]
RML201801a_modulation_Difficult_list = list(range(0, 24))

RML201801a_modulation_list = get_modulation_array_list(RML201801a_modulation_Difficult_list)
classes_dic = filter_classes_str_dic(RML201801a_modulation_Difficult_list)
print('classes_dic:', classes_dic)
num_classes = 24  #别他马动这里
train_dataset, valid_dataset = ModulationRecognitionDataset_RML201801A.create_datasets(train_filename,
                                                                                       train_ratio=train_ratio,
                                                                                       select_every=select_every,
                                                                                       snr_list=snr_list,
                                                                                       modulation_list=RML201801a_modulation_list)
# 保存索引
torch.save(train_dataset.indices, 'train_indices_{}.pt'.format(select_every))
torch.save(valid_dataset.indices, 'valid_indices_{}.pt'.format(select_every))
print('保存成功')
# 加载索引并生成数据集
train_indices = torch.load('train_indices_{}.pt'.format(select_every))
valid_indices = torch.load('valid_indices_{}.pt'.format(select_every))
print('调用成功')
train_dataset = ModulationRecognitionDataset_RML201801A(train_filename, train_indices)
valid_dataset = ModulationRecognitionDataset_RML201801A(train_filename, valid_indices)
print('加载成功')
...
for epoch in range(num_epochs):
    ...
    with tqdm(enumerate(train_loader), total=len(train_loader), unit='batch') as train_progress_bar:
        for batch_idx, (data, target, snr) in train_progress_bar:
            ...
            if dataset_type == 'RML2018.01a':
                target_labels = torch.argmax(target, dim=1)  # 转换为类别标签
...
for snr_show in snr_list:
    ...
    # 测试集上进行预测
    with torch.no_grad():
        for data, target, snr in valid_loader:
            ...
            if dataset_type == 'RML2018.01a':
                target_labels = torch.argmax(target_test, dim=1)  # 转换为类别标签
