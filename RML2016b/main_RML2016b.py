...
from RML2016b_filter_classes_str_dic import convert_labels_to_dict, tensor_to_labels
from getdataset_RML2016b import ModulationRecognitionDataset_RML2016b
...
dataset_type = 'RML2016b'

if dataset_type =='RML2016b':
    print('RML2016b')
    select_every = 1
    # 建议为500的约数
    train_name = './train_new.h5'
    test_name = './val_new.h5'

    snr_list = list(range(-20, 18 + 1, 2))
    RML2016b_list = list(range(10))
    #RML2016b_list = [2,4,6,7,9]
    classes_dic = convert_labels_to_dict(RML2016b_list)
    print('classes_dic:', classes_dic)
    num_classes = len(classes_dic)
    train_dataset = ModulationRecognitionDataset_RML2016b(train_name, select_every, snr_list, RML2016b_list)
    valid_dataset = ModulationRecognitionDataset_RML2016b(test_name, select_every, snr_list, RML2016b_list)
# if dataset_type == 'RML2016a':
#     print('RML2016a改版')
#     train_filename = './data.h5'
#     snr_list = list(range(-20, 18 + 1, 2))
#     RML2016a_list = list(range(11))
#     train_ratio = 0.8
#     select_every = 2

#     classes_dic = convert_labels_to_dict(RML2016a_list)
#     print('classes_dic:', classes_dic)
#     num_classes = len(classes_dic)
#     train_dataset, valid_dataset = ModulationRecognitionDataset_RML2016a.create_datasets(train_filename,
#                                                                                            train_ratio=train_ratio,
#                                                                                            select_every=select_every,
#                                                                                            snr_list=snr_list,
#                                                                                            modulation_list=RML2016a_list)
...
for snr_show in snr_list:
    ...
    # 保存混淆矩阵为PNG格式
    pic_filename = f"{network_name}+{dataset_type}_snr_{snr_show}_test_acc_{test_accuracy:.4f}.eps"
    plt.savefig(pic_filename, bbox_inches='tight',format='eps')
    plt.close()
print("测试结束！！！！！！！！！！！！")
...
# 保存混淆矩阵为PNG格式
pic_filename_all = f"{network_name}+{dataset_type}_overall_test_acc_{test_accuracy_all:.4f}.eps"
plt.savefig(pic_filename_all, bbox_inches='tight',format='eps')
plt.close()
